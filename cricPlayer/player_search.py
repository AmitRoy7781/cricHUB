...
def print_all():
    ...
    print("Bowling Style: ",player_bowlng_style)
    print()
...
if url=="":
    print("Sorry No Data Found, Check Player's Name Spelling")
    exit(0)
...
for i in range(2,len(temp),1):
    ...
    if x.text == "Bowling Style":
        player_bowlng_style = temp[i+1].text
    if x.text == "Bowling Style":
        player_bowlng_style = temp[i+1].text
    if x.text == "Batting":
