...
class Adapter(Target):
    ...
    def news_headline(self):
        self._adaptee.show_news()

    def more_news(self):
        self._adaptee.more_news()

    def news_details(self):
        self._adaptee.detail_news()
class Adaptee:
    """
    ...
    def more_news(self=None):
        page_no = int(request.form["more_news"])
        print(page_no)
        return Adaptee.show_news(str(page_no+1))


    @app.route("/banglanews/detail_news",methods=['POST', 'GET'])
    def detail_news(self=None):
